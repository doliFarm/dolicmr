 ...
 * Copyright (C) 2022 Luigi Grillo <luigi@mandalaorganicgrowers.com>
 *
...
/**
 * \file    test/phpunit/CMRFunctionalTest.php
 * \ingroup cmr
 ...
 * @package Testcmr
 */
class CMRFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	{
		$this->url('/custom/cmr/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('cmr/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/cmr/admin/about.php');
		$this->authenticate();
		return $this->assertContains('cmr/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('cmrwidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modCMR_CMRTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modCMR_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
